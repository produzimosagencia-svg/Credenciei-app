...
test('quem não tem permissão de escanear é recusado', async () => {
  ...
  await assert.rejects(
    () => registrarPorQr(repo, SEGREDO, supervisor.id, 'ev-hj', cracha('evento'), new Date('2026-09-05T19:00:00-03:00')),
    /permissão/,
  )
})
...
test('admin de uma organização não escaneia evento de outra', async () => {
  ...
  await assert.rejects(
    () => registrarPorQr(repo, SEGREDO, admin.id, 'ev-outra', cracha('evento'), new Date('2026-09-05T19:00:00-03:00')),
...
test('QR com assinatura inválida é recusado', async () => {
  ...
  const r = await registrarPorQr(repo, SEGREDO, admin.id, 'ev-hj', 'c3.token.M.assinaturaFalsa', new Date('2026-09-05T19:00:00-03:00'))
...
test('crachá de outro evento não passa — mesmo com assinatura válida', async () => {
  ...
  const r = await registrarPorQr(repo, SEGREDO, admin.id, 'ev-hj', codigoDoRui, new Date('2026-09-05T19:00:00-03:00'))
  assert.equal(r.situacao, 'recusado')
...
test('crachá da etapa errada é recusado nomeando as duas etapas', async () => {
  ...
  const r = await registrarPorQr(repo, SEGREDO, admin.id, 'ev-hj', cracha('montagem'), new Date('2026-09-05T19:00:00-03:00'))
...
test('dia que não está marcado como dia de trabalho recusa a batida', async () => {
  ...
  const r = await registrarPorQr(repo, SEGREDO, admin.id, 'ev-hj', cracha('montagem'), new Date('2026-08-20T10:00:00-03:00'))
  assert.equal(r.situacao, 'recusado')
  if (r.situacao !== 'recusado') return
...
test('o dia do evento respeita a janela configurada', async () => {
  ...
  const cedo = await registrarPorQr(repo, SEGREDO, admin.id, 'ev-hj', cracha('evento'), new Date('2026-09-05T05:00:00-03:00'))
...
test('o crachá certo registra a entrada, e a saída não exige mais o meio', async () => {
  ...
  const entrada = await registrarPorQr(repo, SEGREDO, admin.id, 'ev-hj', cracha('evento'), new Date('2026-09-05T19:00:00-03:00'))
  ...
  const saida = await registrarPorQr(repo, SEGREDO, admin.id, 'ev-hj', cracha('evento'), new Date('2026-09-06T02:00:00-03:00'))
...
test('leitura em sequência, dentro da carência, é recusada — não vira saída por engano', async () => {
  const { repo, admin } = cenarioHenriqueEJuliano()
  await registrarPorQr(repo, SEGREDO, admin.id, 'ev-hj', cracha('evento'), new Date('2026-09-05T19:00:00-03:00'))
  const r = await registrarPorQr(repo, SEGREDO, admin.id, 'ev-hj', cracha('evento'), new Date('2026-09-05T19:01:00-03:00'))
  assert.equal(r.situacao, 'recusado')
  if (r.situacao !== 'recusado') return
...
test('sair e voltar no mesmo dia reabre o turno, e apaga a saída anterior', async () => {
  ...
  const { repo, admin } = cenarioHenriqueEJuliano()
  await registrarPorQr(repo, SEGREDO, admin.id, 'ev-hj', cracha('montagem'), new Date('2026-09-03T08:00:00-03:00'))
  await registrarPorQr(repo, SEGREDO, admin.id, 'ev-hj', cracha('montagem'), new Date('2026-09-03T18:00:00-03:00')) // saída

  const antesDeReabrir = (await repo.registrosDaParticipacao('part-joao')).length
  const r = await registrarPorQr(repo, SEGREDO, admin.id, 'ev-hj', cracha('montagem'), new Date('2026-09-03T18:10:00-03:00'))
...
test('participação inativa é recusada, mesmo com o crachá certo', async () => {
  ...
  const r = await registrarPorQr(repo, SEGREDO, admin.id, 'ev-hj', cracha('evento'), new Date('2026-09-05T19:00:00-03:00'))
  assert.equal(r.situacao, 'recusado')
  if (r.situacao !== 'recusado') return
