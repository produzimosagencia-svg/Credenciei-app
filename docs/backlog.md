...
| 19 | Gastos (produto do Produtor) | 6 | 6 | — | Backend inteiro pronto e testado (22/09) — papel `produtor`, lançamento manual e por voz, lista+filtros, painel e exportação `.xlsx`. Tela construída em 22/09 (captura, lista com filtros e dashboard); falta só a captura por VOZ, que depende de decisão do Juan sobre dependência nativa de áudio — ver "Bloqueado" |
...
- **Decidir a captura de gasto por voz** (Epic 19) — é o coração do módulo no site ("aperta o botão, fala, confirma"), e o backend (`transcreverAudioDeGasto`) já está pronto. Falta o app GRAVAR: exige dependência nativa nova (`expo-audio`, já que `expo-av` saiu de linha no SDK 54+) e permissão de microfone, que mudam o build e o que a pessoa vê ao instalar. Não é decisão técnica — é do Juan. Depende também de `GEMINI_API_KEY` no Render (item abaixo), senão a transcrição responde erro amigável.
