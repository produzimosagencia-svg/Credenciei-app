...
| 3 — Fechar os ciclos pela metade | 2, 4, 6, 7, 8, 9, 10 | 12 | QR (Ed25519, código giratório) e histórico. **Epic 8 fechou em 18/09** (7/7), **Epic 7 em 21/09** (13/13), **Epic 2 e Epic 4 em 22/09** (9/9 e 6/6) e **Epic 10 já estava fechada** (19/19) — todas remapeadas sem tarefa concreta sobrando. Resta só Epic 6 (5, Ed25519/código giratório — travado esperando tempo em produção) e Epic 9 (7, ainda sem lista detalhada) |
...
| 2 | Fundação | 9 | 9 | ✓ | **Completa** — remapeada em 22/09, mesmo método que fechou Epic 4/7/8. Workspace, sincronização, `janelas`/`format`/`tz` portados, `credencial-qr`, teste de assinatura, suíte rodando, e os três papéis que faltavam (`operador_portao`, `suporte`, `produtor`) no domínio. O catálogo "Funções ligadas" (3 camadas de permissão) já estava contado na Epic 17, não aqui — nenhuma lacuna de verdade sobrando |
...
| 9 | Histórico | 5 | 12 | — | Meus dias, Meu pagamento e Meu histórico (entre eventos, 22/09) prontos — os três COM tela desde 22/09 (`/meu-historico` foi a última). Número não recontado: esta epic nunca teve lista tarefa a tarefa |
...
| 14 | Segurança | 11 | 16 | — | Isolamento, limite e a trilha de auditoria feitos. Retenção de foto (21/09), "excluir minha conta" (21/09), consentimento + revogação da busca regional e "baixar meus dados"/portabilidade (22/09) construídos — falta o resto do LGPD, ainda sem lista fechada de itens |
...
>
> **22/09/2026 — Epic 2 (Fundação) remapeada e fechada em 9/9, mesmo dia.**
> Sobravam 4 tasks sem checklist, depois de contar os três papéis que
> faltavam (`operador_portao`, `suporte`, `produtor`, todos prontos hoje).
> Suspeitei que o catálogo "Funções ligadas" (3 camadas de permissão)
> pudesse ser a peça que faltava — checando o histórico, já estava
> contado na Epic 17 desde 13/09/2026, não aqui. Sem mais nada de fundação
> genuinamente pendente (workspace, sincronização, portação de
> `janelas`/`format`/`tz`, `credencial-qr`, teste de assinatura, suíte
...
>
> **22/09/2026 — revogar o consentimento da busca regional, sem excluir a
> conta.** Revisando o que tinha acabado de construir, achei que só dava
> pra AUTORIZAR aparecer na busca regional — não tinha caminho de volta
> sem apagar a conta inteira (`excluirMinhaConta`). LGPD pede que revogar
> consentimento seja tão fácil quanto dá-lo; confirmado com o Juan antes
> de construir.
>
> `POST /v1/minha-conta/revogar-consentimento`
> (`revogarConsentimentoDeBase`) limpa `consentimento_base` em TODOS os
> cadastros com o mesmo CPF, não só a participação que autorizou — a
> busca regional agrupa por pessoa (`todasAsPessoasDaBase`), então
> autorizar ou revogar precisa valer pra ela inteira. Idempotente: revogar
...
- ~~Configurar um agendador externo~~ → **feito em 23/09/2026, e sem serviço externo nenhum.** Com a API no VPS da Hostinger, virou `crontab` no próprio servidor: `/etc/cron.d/credenciei` chama as seis rotas de lembrete a cada 15 min e a limpeza de foto vencida às 04:17. Testado à mão antes de agendar. O cron-job.org não é mais necessário — ver `docs/decisoes/010-api-no-vps-hostinger.md`.
...
- ~~**Ainda falta a tela no app**~~ → **construídas em 22/09/2026**, as cinco: mural "Avisos" (modal na credencial e na equipe do setor), Central de Avisos (`/avisos` — histórico + preferências), módulo Gastos (`/` do produtor, `/gastos-lista`, `/gastos-painel`), Meu histórico (`/meu-historico`) e a "Zona de risco" ampliada em `mais.tsx` (baixar meus dados + revogar consentimento). **A única parte que ficou de fora é a captura de gasto por VOZ** — ver o item logo abaixo.
- ~~Decidir a captura de gasto por voz~~ (Epic 19) → **aprovada e construída em 22/09/2026.** Entrou `expo-audio` (dependência nativa) e a permissão de microfone, com o texto explicando que só grava quando a pessoa aperta. O módulo Gastos está inteiro no app: falar, digitar, listar com filtros, painel e exportar. **Continua dependendo de `GEMINI_API_KEY` no Render** (item abaixo) — sem ela a transcrição responde o erro amigável que a tela já mostra, e só o lançamento digitado funciona.
