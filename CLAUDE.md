...
backlog** (332 tasks, 211 no MVP) — e o do MVP é o que responde "quando dá para usar"; `npm run backlog` calcula os dois — nunca invente número. Login de painel (`entrarComSenha`), o Painel (`painel()`), Escanear QR (`registrarPorQr`, `conferirPorCpf`) e o Ponto assistido (`localizarPessoa`, `abrirFicha`, `registrarPresencaAssistida`) já são reais na API, ligados pelo `ClienteHttp`; o resto das rotas ainda usa o servidor falso — ver a Fase 2 em `docs/backlog.md`. O que está feito por
